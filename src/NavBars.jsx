...
const NavBars = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = data.map((el) => document.getElementById(el.id));
      ...
      <ul className="sm:flex flex-col hidden gap-3 justify-between max-h-[77%]">
        {data.map((el) => (
          <Link
            ...
            smooth={true}
            duration={500}
            offset={-50} // Adjust the offset to your navbar height
            className={`p-4 min-h-12 min-w-12 rounded-full duration-300 ease-in-out w-fit flex gap-2 justify-center items-center ${
              activeId === el.id ? "bg-blue-700 text-white font-bold" : "bg-gray-200"
            }`}
          >
            <li>
              {el.icon}
              <span className={activeId === el.id ? "block" : "hidden"}>
                {el.name}
              </span>
            </li>
          </Link>
